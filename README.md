...
- 🍎 **Apple 健康 / HealthKit 数据桥（待评估）**：不做简单的 H5 套壳，而是评估独立的轻量 SwiftUI 配套应用。在用户明确授权后读取 HRV（SDNN）、睡眠、静息心率、运动后心率恢复、呼吸频率、睡眠腕温和 Cardio Fitness（VO2 Max）等数据，在设备端建立个人基线，仅向跑蓝同步必要的每日摘要，并结合近期训练负荷与天气改善训练准备度、当次训练效果和后续计划建议。实施前先通过 Apple 健康 XML 导出验证分析收益，再决定是否投入自动同步、后端存储和 App Store 分发；需重点评估 HealthKit 原生接入、隐私授权、数据最小化及应用审核成本。
