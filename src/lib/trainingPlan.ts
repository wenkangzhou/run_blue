...
function getFallbackLabels(locale: string = 'zh') {
  ...
  return {
    ...
    raceDay: en ? 'Race Day' : '比赛日',
    taperRaceTips: en ? 'Carb-load 2 days before. Sleep 8h+. No new gear. Light activation day before. Race morning: light breakfast 2-3h before.' : '赛前2天充碳，保证8小时睡眠，不尝试新装备。前一天只做轻量激活。比赛当天：提前2-3小时吃清淡早餐。',
...
function getEasyRunDuration(phase: WeeklyPlan['phase'], w: number, weeks: number, distance: RaceDistance): number {
  const baseEnd = Math.max(1, Math.floor(weeks * 0.25));
  const buildEnd = Math.max(baseEnd + 1, Math.floor(weeks * 0.65));
  if (phase === 'base') {
...
export function generateFallbackTrainingPlan(
  ...
  for (let w = 1; w <= weeks; w++) {
    ...
    // Tue: Easy run
    const tueMin = getEasyRunDuration(phase, w, weeks, distance);
